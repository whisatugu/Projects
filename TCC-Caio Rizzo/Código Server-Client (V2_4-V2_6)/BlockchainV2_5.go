		...
		switch op {
            ...
            case "GAMESTATE":
            	...
            	if err != nil {
            		...
            		fmt.Println("Nome do user: ", m.Values[0].Username)
            		fmt.Println("Hash da Senha: ", m.Values[0].PasswordHash)
            		fmt.Println("Ether: ", m.Values[0].Ether)
            	}


   /*         	var f interface{}
            	err := json.Unmarshal([]byte(args[1]), &f)
            	if err != nil {
            		fmt.Println("Error na decodificação do Json")
            	} else {
