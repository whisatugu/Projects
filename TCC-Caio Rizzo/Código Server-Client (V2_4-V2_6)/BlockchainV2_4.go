...
var close = make(chan *Client)

//Cria o canal de saída
var att = make(chan int, 2)
...
func calculateBlockHash(block Block) string {
	...
	record := string(block.Head.Index) + block.Head.TableHash + block.Head.CommandsHash + block.Head.PrevHash + block.Head.Validator + string(t.Second())
...
func generateBlock(oldBlock Block, address string, commands []string) (Block, error) {
	...
	newBlock.Head.TableHash = "Work on Progress"
...
func decodificaTabela(b []byte) ([][]string){
    ...
    err := dec.Decode(&mat)
    if err != nil {
        log.Fatal("decode table error:", err)
    }
	...
	for i := 0; i < qtd; i++ {
		for j := range randomList {
		    ...
		    for e := connectionsList.Front(); e != nil; e = e.Next() {
		    	//fmt.Println("Ip: " + e.Value.(*Client).IP + " " + ip[0])
	 			if ip[0] == e.Value.(*Client).IP {
	 				flagIgual = 1
	 				break
	 			}
	 		}
...
func server(exit chan string){
	...
	go func(){
		for {
			...
			for e := connectionsList.Front(); e != nil; e = e.Next() {
				...
				fmt.Println("Cliente ", e.Value.(*Client))
	 			if empty == e.Value.(*Client) {
	 				...
	 				fmt.Println("Número de clientes conectados: ", connectionsList.Len())
	 			}
	 		}
 		}
	}()
	...
	for {
		...
		for i = 0; i < MAXCONNS; i++ {
			...
			localIP := strings.SplitN(aux, ":", 2)
			c.IP = localIP[0]

			//Verifica se não é o ip de uma conexão vigente
		    flagIgual := 0
		    for e := connectionsList.Front(); e != nil; e = e.Next() {
		    	fmt.Println("Ip: " + e.Value.(*Client).IP + " " + c.IP)
	 			if c.IP == e.Value.(*Client).IP {
	 				flagIgual = 1
	 				break
	 			}
	 		}
...
func distribuiBloco(client *Client, bloco Block) {
	//c1 := make(chan string, 1)
	for e := connectionsList.Front(); e != nil; e = e.Next() {
		enviar := e.Value.(*Client)
		if enviar != client {
			enviar.Mutex.Lock() //Para envitar enviar uma mensagem enquanto o cliente se encontra no meio de uma requisição
			if enviar.Conn != nil {
...
func handleConnection(client *Client, close chan *Client, i int) {
...
func interfaceTeste() {
	...
	for{
		...
		case 1 :
			...
			for e := connectionsList.Front(); e != nil; e = e.Next() {
				fmt.Println("Cliente ", (e.Value.(*Client)).IP)
	 		}
...
func generateGenesisBlock() {
	...
	genesisBlock.Head.TableHash = "First of Many"
