...
-Substituidos Commands por Transactions nos blocos
-Implementado sistema de Flooding com Map de players e controle por SeqNumber
...
func calculateDifficultyPOS(newBlock Block, oldBlock Block) (int) {
	...
	if newBlock.Transactions[0].Commands != nil {
		//Captura o Ether
		//Sempre a primeira transação do bloco
...
func generatePOS() (Block, bool) {
	...
	go floodTransaction(player.C, t)
	...
	for tentativas := 1; ; tentativas++ {
		...
		newBlock, _ = generateBlock(oldBlock, t.Username, tVet)
	...
	for{
		...
		switch op {
			...
			case "TRANSACTION":
				//TRANSACTION BYTES\n
				client.Mutex.Lock()
				t := decodificaTransaction(strings.TrimSpace(args[1]))
				client.Conn.Write([]Byte("OK\n"))
				client.Mutex.Unlock()

				if verificaTransacao(t) {
					mutexTransactionPool.Lock()
            		transactionPool.PushBack(args[1]) //Coloca o comando no pool
            		mutexTransactionPool.Unlock()
			...
			case "SEQNUMBER":
				//Desencampsula mensagem
				//SEQNUMBER I TRANSACTION BYTES\n
				args2 := strings.SplitN(args[1], " ", 2)

				//Verifica o número de sequencia da mensagem para saber se é repetida
				//if client.SeqNumber > strconv.Atoi(args2[0]){
					//Pode ser comando de TRANSACTION ou de REGISTER

					//Coloca na lista para próximo bloco

					//Redistribui para os clientes conectados
					distribuiMsg(client, args2[1])
				//}
