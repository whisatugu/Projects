...
-Corrigido bug em transações enviadas
...
func codifica2(block Block) ([]byte){
...
func handleGameState(client *Client, jsonS string){
	...
	if carregaPlayersFromJson(gameState){
		...
		fmt.Println("Enviando ERROR na funão HandleGameState")
		w.WriteString("ERROR 1\n")
		w.Flush()
	}
	...
	for{
		...
		switch op {
            ...
            case "WIN":
				...
				fmt.Println("================================= ", args)
            	b := decodifica2([]byte(strings.TrimSpace(args[1])))
