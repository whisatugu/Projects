...
-Mensagens entre servidor e jogo agora tem ACK de recebimento
-Comando Explore
-Bugs corrigidos
...
func enviaComandosParaJogo(bloco Block) {
	for i := 0; i < len(bloco.Transactions); i++ {
		for j := 0; j < len(bloco.Transactions[i].Commands); j++ {
			if bloco.Transactions[i].SeqNumber !=0 { //Se é o registro pula
...
func handleGameState(client *Client, jsonS string){
	...
	fmt.Println("Enviando OK na funão HandleGameState")
	w.WriteString("OK\n")
	w.Flush()
		...
		switch op {
			...
			case "TRANSACTION":
				...
				if verificaTransacao(t) {
            		...
            		mutexTransactionPool.Unlock()
					go distribuiMsg(client, message)
				}
