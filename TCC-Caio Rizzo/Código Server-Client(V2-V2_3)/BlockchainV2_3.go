...
func generateBlock(oldBlock Block, address string) (Block, error) {
	...
	newBlock.Head.TableHash = string(codificaComandos()) //adicionar hash da tabela
...
func calculateCommandsHash() (string) {
	//Transforma os comandos em um vetor de bytes
	bytes := codificaComandos()
...
func generatePOS() (Block, bool){
	...
	for tentativas := 1; ; tentativas++ {
		...
		newBlock, _ = generateBlock(oldBlock, localIP[0])
...
func interfaceTeste() {
	...
	for{
		...
		case 5 :
			b := codificaComandos()
			fmt.Println("Codificação da POOL: ", b)
